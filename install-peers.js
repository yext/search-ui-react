...
function installPeerDeps() {
  ...
  getPackageConfig(rootPath, function(config) {
    if (config === null) {
      console.log('@yext/answers-headless-react will not be automatically installed because @yext/answers-react-components is not being installed as a node_module of another package');
      return;
    }

    var peerDeps           = ['@yext/answers-headless-react@^1.1.0-beta.11']

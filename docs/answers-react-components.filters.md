...
|  [HierarchicalFacet({ facet, divider, showMoreLimit, customCssClasses, cssCompositionMethod })](./answers-react-components.filters.hierarchicalfacet.md) | A HiearchicalFacet takes a <code>DisplayableFacet</code> and renders the facet in a way to represent multiple levels of "hierarchies".<!-- -->The hierarchies are determined by the provided divider, which defaults to "<!-- -->&gt;<!-- -->". |
